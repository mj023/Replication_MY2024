...
def simulate_moments(params):
    ...
    for health in range(2):
        for interval in range(4):
            working_pct_10years = (res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5))  & (res['health'] == health), ["working"]].sum()/2)/(res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5))   &(res['health'] == health), "health"].count())
            moments[(interval+4*(1-health))] = working_pct_10years.iloc[0]
    for health in range(2):
        for education in range(2):
            for interval in range(6):
                avg_effort_10years = (res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5))  & (res['health'] == health)& (res['education'] == education), ["effort"]].sum())/(res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5)) &(res['health'] == health)  & (res['education'] == education), "effort"].count())
                moments[(interval+6*(1-health)+education*6*2)+8] = avg_effort_10years.iloc[0]
                if interval < 4:
                    avg_income_10years = (res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5))  & (res['health'] == health)& (res['education'] == education), ["income"]].sum())/(res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5)) &(res['health'] == health)  & (res['education'] == education), "income"].count())
                    moments[(interval+4*(1-health)+education*4*2)+46] = avg_income_10years.iloc[0]*winit[1]/1000
    for interval in range(6):
                median_wealth_10y = res.loc[(res['period'] >= (interval*5)) & (res['period'] < ((interval+1)*5)) , ["wealth"]].median()
                moments[interval+32] = median_wealth_10y.iloc[0]
    avgemp_HS = (res.loc[ (res['education'] == 0), ["working"]].sum()/2)/(res.loc[ (res['education'] == 0), "working"].count())
    avgemp_CL = (res.loc[ (res['education'] == 1), ["working"]].sum()/2)/(res.loc[ (res['education'] == 1), "working"].count())
    moments[38] = avgemp_CL.iloc[0]/avgemp_HS.iloc[0]
    for interval in range(3):
        non_adjusters = (res.loc[(res['period'] >= (interval*10)) & (res['period'] < ((interval+1)*10)) & (res['effort'] == res['effort_t_1'])].count())/ (res.loc[(res['period'] >= (interval*10)) & (res['period'] < ((interval+1)*10)) ].count())
    ...
    cons_ratio = (res.loc[ (res['health'] == 1), 'cnow'].sum()/res.loc[ (res['health'] == 1), 'cnow'].count())/ (res.loc[ (res['health'] == 0), 'cnow'].sum()/res.loc[ (res['health'] == 0), 'cnow'].count())
    moments[45] = cons_ratio
    log_earnings = np.log(res.loc[ (res['period']<= retirement_age) & (res['working'] > 0), 'income'] * theta_val[1])
    moments[62] = log_earnings.var()
    pension_avg = (res.loc[ (res['period'] == retirement_age + 1), 'pension'].sum()/ res.loc[ (res['period'] == retirement_age+1), 'pension'].count())
