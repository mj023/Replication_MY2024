...
from lcm import DiscreteGrid, LinSpacedGrid, MarkovTransition, AgeGrid, Model, Regime, categorical
from lcm.typing import BoolND, ContinuousAction, ContinuousState, DiscreteAction, DiscreteState, FloatND, Period
import lcm
...
# Utility function
# --------------------------------------------------------------------------------------
def utility(scaled_adjustment_cost: FloatND, fcost: FloatND, disutil: FloatND, cons_util: FloatND) -> FloatND:
    return cons_util - disutil - fcost - scaled_adjustment_cost
def disutil(working: DiscreteAction, health: DiscreteState, education: DiscreteState, period: Period, phigrid: FloatND) -> FloatND:
    return phigrid[period,education,health] * ((working/2)**(2))/2
def scaled_adjustment_cost(period: Period, adjustment_cost: ContinuousState, effort: DiscreteAction, effort_t_1: DiscreteState, chimaxgrid: FloatND) -> FloatND:
    cost = jnp.where(jnp.logical_not(effort == effort_t_1), adjustment_cost*chimaxgrid[period], 0)
    return cost
def cnow(net_income: FloatND, wealth: ContinuousState, saving: ContinuousAction) -> FloatND:
    wealth = calc_savingsgrid(wealth)
    saving = calc_savingsgrid(saving)
    cnow = jnp.maximum(net_income + (wealth)*r - (saving), mincon)
    return cnow
def cons_util(health: DiscreteState, cnow: FloatND, kappa: float, sigma: float, bb: float) -> FloatND:
    mucon = jnp.where(health, 1, kappa)
    return mucon*(((cnow)**(1.0-sigma))/(1.0-sigma)) + mucon*bb
def fcost(period: Period, education: DiscreteState, health: DiscreteState, effort: DiscreteAction, psi: float, xigrid: FloatND) -> FloatND:
...
def scaled_productivity_shock(productivity_shock: ContinuousState, sigx: float) -> FloatND:
    return productivity_shock * sigx
def income(working: DiscreteAction, period: Period, health: DiscreteState, education: DiscreteState, productivity: DiscreteState, scaled_productivity_shock: FloatND, income_grid: FloatND) -> FloatND:
    return income_grid[ period, health, education]*(working/2)*theta_val[productivity]*jnp.exp(scaled_productivity_shock)
...
def pension(period: Period, education: DiscreteState, productivity: DiscreteState, income_grid: FloatND, penre: float) -> FloatND:
...
def next_wealth(saving: ContinuousAction) -> ContinuousState:
    return saving
def next_health(period: Period, health: DiscreteState, effort: DiscreteAction, effort_t_1: DiscreteState, education: DiscreteState, health_type: DiscreteState, probs_array: FloatND) -> FloatND:
...
def next_regime(period: Period, education: DiscreteState, health: DiscreteState, probs_array: FloatND) -> FloatND:
...
def savings_constraint(net_income: FloatND, wealth: ContinuousState, saving: ContinuousAction) -> BoolND:
    wealth = calc_savingsgrid(wealth)
    saving = calc_savingsgrid(saving)
